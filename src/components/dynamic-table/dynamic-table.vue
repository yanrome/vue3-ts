  ...
  <div v-if="listType==='table'">
    <a-table :columns="columns"
      ...
      <template v-for="slotItem in columns.filter(item => item.slots)"
        ...
        <!--        自定义渲染start-->
        <slot v-if="$slots[slotItem.slots.customRender]"
          ...
          <div v-if="slotItem.slots.customRender == 'action'"
            ...
            <template v-for="(action, index) in actions">
              <template v-if="action.type == 'select'">
                <!--              下拉选择器-->
                <a-select v-model:value="slotProps.record[action.key]"
                          :key="index"
                          size="small">
                  <Option v-for="option in action.options"
                          :value="option.value"
                          :key="option.value">
                    {{ option.label }}
                  </Option>
          ...
          <div class="w100 u-a-item-main flex-bet">
            ...
            <div class="u-aim-item u-aim-group-item z-text-center">
              ...
              <div class="u-aim-more">
                <a-button class="u-aim-more-btn u-aim-before"
                          @click="navDetails(item.id)"
                          type="link">
                  查看详情
                </a-button>
...
export default {
  ...
  setup(props: Props, { attrs, emit, slots }) {
    ////
    const { pageOption } = Object.assign(
      usePages({
        pageChangeFn: (data) => {
          state.pageOption = {
            ...state.pageOption,
    ...
    const refreshTableData = async (params = {}) => {
      ...
      state.loading = true
      const { data, msg, ret, total } = await props
        .getListFunc(params)
