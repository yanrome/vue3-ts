<template>
  ...
  <a-card>
    <dynamic-table :pageOption="pageOption"
                   rowKey="id"
...
export default defineComponent({
  ...
  setup() {
    ...
    const getDictFn = async () => {
      ...
      let payment = await getDict('business_payment', 'businessPayment', false)

      let refundStatus = await getDict('business_refund_status', '', false)
      return {
        refundType: data,
        payment: payment,
    ...
    const confirm = () => {
      ...
      if (dynamicForm.value.modelRef.time) {
        dynamicForm.value.modelRef.time = dynamicForm.value.modelRef.time.map(
          (item) => {
            return moment(item).format('YYYY-MM-DD')
          }
        )
      }
      state.pageOption = Object.assign(
        state.pageOption,
    ...
    const cancel = () => {
      ...
      dynamicForm.value.modelRef = Object.assign(
        dynamicForm.value.modelRef,
        nothing
      )
    }
